...
if isfield(mooring,'block')
    if(Environment.add_spectra)
        disp("start sliding spectra")
        mooring  = sliding_spectra(mooring,Environment);
        disp("done sliding spectra")
    end
    if(Environment.add_KEpeak)
        disp("start identifying main peaks")
        mooring  = get_KE_peak(mooring,Environment);
        disp("done identifying main peaks")
    end
    if(Environment.add_GM)
        disp("start getting GM")
        mooring  = get_GM(mooring,Environment);
        disp("done getting GM")
    end
    if(Environment.add_NIshape)
        disp("start getting NI shape" )
        mooring  = get_NIshape(mooring);
        disp("done getting NI shape")
    end
